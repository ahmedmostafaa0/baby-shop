...
export const AddressSelection: React.FC<AddressSelectionProps> = ({
  ...
  const handleAddAddress = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const { data } = await api.post(`/users/${user._id}/addresses`, formData);
  ...
  const handleEditAddress = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user || !isAuthenticated || !editingAddress) return;

    setIsLoading(true);
    try {
      const { data } = await api.put(
        `/users/${user._id}/addresses/${editingAddress._id}`,
        formData,
      );
  ...
  const handleDeleteAddress = async (addressId: string) => {
    ...
    try {
      const { data } = await api.delete(
        `/users/${user._id}/addresses/${addressId}`,
      );
      onAddressesUpdate(data.addresses);
      toast.success(data.message);
    } catch (error) {
