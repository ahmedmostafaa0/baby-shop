...
const SuccessPageClient = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          console.log("Success: Order fetched:", data);

          // Handle payment status update
          if (
            sessionId &&
            needsPaymentUpdate(data, sessionId) &&
            !statusUpdated
          ) {
          ...
          } else if (data.status === "paid" && sessionId) {
    ...
    fetchOrder();
  }, [orderId,  router,  sessionId, statusUpdated]);
  ...
  useEffect(() => {
    ...
    startPolling();
  }, [order, sessionId, statusUpdated, orderId]);
